...
module.exports = function(context) {
    ...
    function checkCallee(expr, node) {
        if (expr.callee.object.type === 'ThisExpression') {
    ...
    return {
        ...
        'CallExpression': function(node) {
            ...
            if (node.parent.type === 'ExpressionStatement' && _.isArray(node.parent.parent.body)) {
                var positionWithinParent = node.parent.parent.body.indexOf(node.parent);
        ...
        'ExpressionStatement': function(node) {
            ...
            if (expr.type === 'CallExpression') {
                if (expr.callee && expr.callee.object) {
                    checkCallee(expr, node);
                }

                if (expr.arguments) {
                    _.each(expr.arguments, function (arg) {
                        if (arg.callee) {
                            checkCallee(arg, node);
                        }
                    });
                }
            }
