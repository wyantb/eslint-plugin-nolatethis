...
module.exports = function(context) {
    ...
    function findOwningFunctionDeclaration(node) {
        while (node && node.type !== 'FunctionDeclaration' && node.type !== 'FunctionExpression') {
            node = node.parent;
        }
    ...
    return {
        'VariableDeclaration': function(node) {
            if (!hasParentWithSafeContext(findOwningFunctionDeclaration(node))) {
                return;
            }
        ...
        'IfStatement': function(node) {
            ...
            checkForThisArguments(node.test, true, node);
        },
